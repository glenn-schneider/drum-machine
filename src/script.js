  ...
  render() {
    return (
      <div className="container" id="drum-machine">
        ...
        <h3 className="text-center" id="display">{this.state.lastSound}</h3>
        <div className="row">
          <div className="col text-center">
            {drumButtons.map( (btn, index) => (
              <button className="btn drum-pad btn-primary" onClick={() => this.playAudio(btn.key, btn.sound)} id={btn.sound}>
                {btn.key}
                <audio className="clip" src={btn.url} id={btn.key} />
              </button>
            ))}
          </div>
        </div>
